...
namespace ChallengeCore.Challenges
{
    ...
    {
        [Challenge("Advent of Code 2020", "Custom Customs (6)", "https://adventofcode.com/2020/day/6")]
        public class CustomCustoms : IChallenge
        {
            public void Solve()
            {
                var sum1 = 0;
                ...
                while ((answers = GetGroupAnswers()) != null)
                {
                    var allAnswers = new HashSet<char>();
                    HashSet<char> sect = null;

                    foreach (var answerLine in answers)
                    {
                        if (sect == null)
                        {
                            sect = new HashSet<char>(answerLine);
                        }
                        else
                        {
                            sect.IntersectWith(new HashSet<char>(answerLine));
                        }
                        foreach (var answer in answerLine)
                        {
                            allAnswers.Add(answer);
                        }
                    }

                    sum1 += allAnswers.Count();
                    sum2 += sect.Count();
                }
            ...
            private List<string> GetGroupAnswers()
            {
                ...
                var answers = new List<string>() {nextLine};
