...
namespace ChallengeCore.Challenges
{
    ...
    {
        [Challenge("Advent of Code 2020", "Handy Haversacks (7))", "https://adventofcode.com/2020/day/7")]
        public class HandyHaversacks : IChallenge
        {
            private class ContentsRule
            {
                readonly Dictionary<string, int> counts = new Dictionary<string, int>();

                public void Add(string bag, int count)
                {
                    counts[bag] = count;
                }

                public bool ContainsBag(string bag)
                {
                    return counts.ContainsKey(bag);
                }

                public IEnumerable<string> ContainedBags => counts.Keys;

                public int BagCount(string bag)
                {
                    return !counts.ContainsKey(bag) ? 0 : counts[bag];
                }
            }
            ...
            public void Solve()
            {
                ...
                {
                    var (containingBag, thisRule) = GetRule(rule);
                    rules[containingBag] = thisRule;
                    foreach (var containedBag in thisRule.ContainedBags)
                    {
                        if (!containers.ContainsKey(containedBag))
                        {
                            containers[containedBag] = new List<string>();
                        }
                        containers[containedBag].Add(containingBag);
                    }
                }
                ...
                foreach (var container in containers["shiny gold"])
                {
                    bagQueue.Enqueue(container);
                }

                while (bagQueue.Count > 0)
                {
                    var nextBag = bagQueue.Dequeue();
                    if (checkedBags.Contains(nextBag))
                    {
                        continue;
                    }

                    checkedBags.Add(nextBag);
                    if (!containers.ContainsKey(nextBag))
                    {
                        continue;
                    }
                    foreach (var container in containers[nextBag])
                    {
                        bagQueue.Enqueue(container);
                    }
                }

                WriteLine(checkedBags.Count);

                // Part 2
                WriteLine(TotalBagsIn("shiny gold"));
            }

            private int TotalBagsIn(string bag)
            {
                ...
                var rule = rules[bag];
                return rule.ContainedBags.Select(b => (TotalBagsIn(b) + 1) * rule.BagCount(b)).Sum();
            }

            private static readonly string RgxBagDesc = Cat(
                LowerLetter.Rep(1),
                " ",
                LowerLetter.Rep(1),
                " bag",
            ...
            private static readonly string RgxRule = Cat(
                RgxBagDesc.Named("containing"),
                " contain ",
                Cat(
                    Integer().Named("count"),
                    " ",
                    RgxBagDesc.Named("contained"),
                    Stex.AnyOf(
                        ", ",
                        "." + End)
                ).Rep(1)
            );
            private static readonly Regex RegexBags = new Regex(RgxRule);
            private static readonly Regex RegexBag = new Regex(RgxBagDesc);

            private (string containingBag, ContentsRule rule) GetRule(string ruleLine)
            {
                var ruleContents = new ContentsRule();
                if (ruleLine.EndsWith("no other bags."))
                {
                    var noBags = RegexBag.Match(ruleLine);
                    var bag = noBags.Value[0..^5];
                    return (bag, ruleContents);
                }
                var match = RegexBags.Match(ruleLine);
                var containing = match.Groups["containing"].Value[0..^5];

                var count = match.Groups["count"].Captures.Select(c => int.Parse(c.Value)).ToArray();
                var contained = match.Groups["contained"].Captures.Select(c => c.Value).ToArray();
                if (contained.Length != count.Length)
                {
                    throw new InvalidDataException();
                }

                for(int iCont = 0; iCont < contained.Length; iCont++)
                {
                    bool fPlural = count[iCont] > 1;
                    string name = contained[iCont][0..^(fPlural ? 5 : 4)];
                    ruleContents.Add(name, count[iCont]);
                }
                return (containing, ruleContents);
            }
