...
namespace ChallengeCore.Challenges
{
    ...
    {
        [Challenge("Advent of Code 2020", "Passport Processing (4)", "https://adventofcode.com/2020/day/4")]
        public class PassportProcessing : IChallenge
        {
            public void Solve()
            {
                var cValids = 0;
                ...
                while ((dict = GetPassportFields()) != null)
                {
                    if (Validate(dict))
                    {
                        cValids++;
            ...
            private string[] expected = new[]
            {"byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"};

            private bool Validate(Dictionary<string, string> dict, bool fDataValidation = false)
            {
                var diff = dict.Keys.Count - expected.Length;
                ...
                if (diff == 1 && !dict.ContainsKey("cid"))
                {
                    return false;
                }
            ...
            private bool IsValidField(Dictionary<string, string> dict, string fld)
            {
                var val = dict[fld];
                ...
                {
                    case "byr":
                        validator = IsValidByr;
                        break;

                    case "iyr":
                        validator = IsValidIyr;
                        break;

                    case "eyr":
                        validator = IsValidEyr;
                        break;

                    case "hgt":
                        validator = IsValidHgt;
                        break;

                    case "hcl":
                        validator = IsValidHcl;
                        break;

                    case "ecl":
                        validator = IsValidEcl;
                        break;

                    case "pid":
                        validator = IsValidPid;
                        break;
                }
            ...
            private bool IsValidDate(string year, int low, int high)
            {
                if (year.Length != 4)
                {
                    return false;
                ...
                if (!int.TryParse(year, out var val))
                {
                    return false;
                }
            ...
            private bool IsValidByr(string byr)
            {
                return IsValidDate(byr, 1920, 2002);
            }

            private bool IsValidIyr(string iyr)
            {
                return IsValidDate(iyr, 2010, 2020);
            }

            private bool IsValidEyr(string eyr)
            {
                return IsValidDate(eyr, 2020, 2030);
            }

            private bool IsValidHgt(string hgt)
            {
                // Default to inches
                int low = 59, high = 76;
                ...
                if (!int.TryParse(hgt[0..^2], out var val))
                {
                    return false;
                }
            ...
            private bool IsValidHcl(string hcl)
            {
                if (hcl[0] != '#')
                {
                    return false;
                ...
                return hcl.Skip(1).All(c => char.IsDigit(c) || c >= 'a' && c <= 'f');
            }

            private static readonly HashSet<string> ValidEcl = new HashSet<string>()
            {
                "amb", "blu", "brn", "gry", "grn", "hzl", "oth"
            };
            private bool IsValidEcl(string ecl)
            {
                return ValidEcl.Contains(ecl);
            }

            private bool IsValidPid(string pid)
            {
                if (pid.Length != 9)
                {
                    return false;
            ...
            private static readonly string SearchFld = Cat(
                LowerLetter.Rep(3, 3).Named("tag"),
                ":",
                Not(White).Rep(1).Named("value"));
            private static readonly Regex RegexFld = new Regex(SearchFld);

            private Dictionary<string, string> GetPassportFields()
            {
                string nextLine = "";

                while ((nextLine = ReadLine()?.Trim()) == string.Empty) ;
                if (nextLine == null)
                {
                    return null;
                }
                ...
                do
                {
                    var matches = RegexFld.Matches(nextLine);
                ...
                } while ((nextLine = ReadLine()?.Trim()) != string.Empty && nextLine != null);
