...
namespace ChallengeCore.Challenges
{
	...
	{
		[Challenge("ProgChallenges", "Stack 'em Up",
			"https://onlinejudge.org/index.php?option=onlinejudge&Itemid=8&page=show_problem&problem=1146")]
		public class StackEmUp : IChallenge
		{
			...
			public class Shuffler
			{
				...
				private List<PokerHands.Card> _deck = new List<PokerHands.Card>();

				public Shuffler()
				{
					var cShuffles = int.Parse(ReadLine());
					var allElements = new List<int>();
					while (allElements.Count() != cShuffles * 52)
					{
						var elements = ReadLine().Split(new[] { ' ' }).Select(int.Parse).ToList();
					...
					while ((shuffleIndexString = ReadLine()) != string.Empty && shuffleIndexString != null)
					{
					...
					foreach (var iSuit in suits)
					{
						...
						{
							_deck.Add(new PokerHands.Card(iSuit, iRank));
						}
					}
				}
				...
				{
					var tempDeck = new List<PokerHands.Card>();
					for (var i = 0; i < 52; i++)
					{
